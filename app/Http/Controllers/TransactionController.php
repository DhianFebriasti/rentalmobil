...
class TransactionController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            DB::commit();
            $request->session()->flash('success-message',"Data telah disimpan dengan estimasi {$data_transaction ['amount']}");
            ...
            return redirect()->route('transaction.create',$transaction->id);
        } catch (\Exception $e) {
            DB::rollback();
    ...
    private function generateInvoice($date){
        ...
        $no = 'SMB'.$tanggal.$bulan.$tahun.'-'.sprintf('%05s',$transaction->count()+1);
        return $no;
    }
